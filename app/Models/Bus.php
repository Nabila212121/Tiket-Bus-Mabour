...
class Bus extends Model
{
    ...
    public function getUsedSeat($date, $scheduleId) : array //untuk mendapatkan daftar nomor kursi yang sudah dipesan
    {
        $date = Carbon::parse($date)->format('Y-m-d');
        return $this->busTicket()->whereIn('status', ['pending', 'approved'])
        ->whereDate('departure_time', $date)
        ->where('bus_schedule_id',$scheduleId)
        ->get()
        ->pluck('seat_number')
        ->toArray();
    }
